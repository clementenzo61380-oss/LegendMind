...
def test_weekly_consistency_full_week_counts() -> None:
    ...
    snapshots = [_snap(base + timedelta(days=i, hours=12), 5500, attacks=10) for i in range(3)]
...
def test_weekly_consistency_partial_week_counts_as_total_only() -> None:
    ...
    snapshots = [_snap(base + timedelta(days=i, hours=12), 5500, attacks=2) for i in range(3)]
    full, total, ratio = weekly_consistency(snapshots, quota=quota)
    assert total == 1
    assert full == 0
    assert ratio == 0.0
...
def test_recap_digest_computes_week_deltas() -> None:
    ...
    snapshots = [
        _snap(base, 5500, attacks=0, gained=0, lost=0, tier=LeagueType.LEGEND_II),
        _snap(
...
def test_recap_digest_handles_attack_counter_reset() -> None:
    ...
    snapshots = [
        ...
        _snap(base + timedelta(days=6), 5550, attacks=5, tier=LeagueType.LEGEND_III),
    ]
    digest = _RecapDigest(
        player_tag="#X",
        discord_user_id=42,
