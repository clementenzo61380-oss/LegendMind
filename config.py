...
def _lifetime_entitled_discord_ids() -> frozenset[int]:
    ...
    for token in raw.replace(",", " ").split():
        ...
        try:
            ...
            _log.warning(
                "LIFETIME_ENTITLED_DISCORD_IDS: jeton ignoré (non entier): %r", t,
            )
...
def load_config() -> Config:
    ...
    return Config(
        ...
        daily_legend_export_minute_after_reset=_int(
            "DAILY_LEGEND_EXPORT_MINUTE_AFTER_RESET", 5,
        ),
        daily_legend_export_xlsx_path=os.environ.get(
            "DAILY_LEGEND_EXPORT_XLSX_PATH", "data/daily_legend_stats.xlsx",
        ),
        daily_legend_export_state_path=os.environ.get(
            "DAILY_LEGEND_EXPORT_STATE_PATH", "data/.daily_legend_export_last",
        ),
